...
<view class="item">
	<view class="first">
		<view class="place">
			...
			<view class="projectName">{{item.date}}</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			toDetail(){
				uni.navigateTo({
					url:`/pages/cutPlayer/cutPlayer?list=${encodeURIComponent(encodeURIComponent(JSON.stringify({...this.item,userId:uni.getStorageSync('user').id})))}`
